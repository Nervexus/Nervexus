...
(function (root) {
  ...
  if (root.LearningScience) return;
  ...
  var PRACTICAL = [
    ...
    { level: 15, q: 'What is a hypothesis in a scientific investigation?', a: 'A testable prediction based on scientific reasoning', d: ['A testable prediction based on scientific reasoning', 'A proven scientific law', 'The final conclusion of an experiment', 'A random guess with no reasoning'], why: 'A hypothesis proposes a testable, reasoned prediction about what will happen, which the experiment is then designed to test.' },
    { level: 18, q: 'What is the difference between precision and accuracy?', a: 'Precision is how consistent repeated measurements are; accuracy is how close they are to the true value', d: ['Precision is how consistent repeated measurements are; accuracy is how close they are to the true value', 'They mean exactly the same thing', 'Precision only applies to digital equipment', 'Accuracy only matters in chemistry experiments'], why: 'A set of results can be precise (tightly clustered) without being accurate (close to the true value) if there is a systematic error, and vice versa.' },
    { level: 22, q: 'What is a systematic error in an experiment?', a: 'A consistent error caused by faulty equipment or method, affecting every reading the same way', d: ['A consistent error caused by faulty equipment or method, affecting every reading the same way', 'A one-off mistake in reading a single result', 'A random fluctuation with no clear cause', 'An error that only occurs once in an entire experiment'], why: 'Systematic errors (e.g. a wrongly calibrated scale) shift every result in the same direction, unlike random errors which scatter results unpredictably.' },
    { level: 30, q: 'Why is a control group used in an experiment?', a: 'To provide a baseline for comparison, showing what happens without the tested variable', d: ['To provide a baseline for comparison, showing what happens without the tested variable', 'To make the experiment take longer', 'To double the amount of equipment needed', 'It is not necessary in a well-designed experiment'], why: 'A control group is not exposed to the independent variable, allowing researchers to compare results and confirm the variable, not something else, caused the change.' },
    { level: 40, q: 'What does it mean for a scientific method to be “valid”?', a: 'It actually tests what it claims to test, with variables properly controlled', d: ['It actually tests what it claims to test, with variables properly controlled', 'It has been repeated many times', 'It uses expensive, high-tech equipment', 'It produces the result the scientist expected'], why: 'A valid experiment is designed so its results genuinely answer the question being investigated, with other variables controlled so they cannot distort the outcome.' },
    { level: 52, q: 'When plotting a graph of experimental results, why is a line of best fit used rather than joining points dot-to-dot?', a: 'It shows the overall trend, smoothing out random error in individual readings', d: ['It shows the overall trend, smoothing out random error in individual readings', 'It makes the graph look more decorative', 'It always passes through every single data point', 'It is only used for straight-line relationships, never curves'], why: 'A line (or curve) of best fit represents the underlying relationship between variables, treating scattered individual points as containing some random measurement error.' }
    ];
  var VOCAB = [
    ...
    { level: 9, q: 'A mixture of substances that are not chemically bonded together is called a...', a: 'Mixture', d: ['Mixture', 'Compound', 'Element', 'Molecule'], why: 'In a mixture, substances are combined physically (not chemically), so each retains its own properties and can usually be separated again.' },
    { level: 17, q: 'An atom of the same element with a different number of neutrons is called an...', a: 'Isotope', d: ['Isotope', 'Ion', 'Isomer', 'Allotrope'], why: 'Isotopes share the same number of protons (and so the same element) but differ in neutron number, giving them a different atomic mass.' },
    { level: 24, q: 'A reaction that releases energy to the surroundings, often as heat, is described as...', a: 'Exothermic', d: ['Exothermic', 'Endothermic', 'Catalytic', 'Reversible'], why: 'Exothermic reactions transfer energy out to the surroundings, usually causing a temperature rise, as in combustion.' },
    { level: 28, q: 'A quantity with both size (magnitude) and direction, like velocity, is called a...', a: 'Vector', d: ['Vector', 'Scalar', 'Constant', 'Coefficient'], why: 'Vector quantities (velocity, force, displacement) need both a magnitude and a direction to be fully described, unlike scalar quantities (speed, distance, mass).' },
    { level: 33, q: 'A community of organisms interacting with each other and their physical environment is an...', a: 'Ecosystem', d: ['Ecosystem', 'Habitat', 'Population', 'Biome'], why: 'An ecosystem includes both the living organisms (the community) and the non-living physical environment they interact with.' },
    { level: 38, q: 'A random change in an organism’s DNA sequence is called a...', a: 'Mutation', d: ['Mutation', 'Adaptation', 'Selection', 'Variation'], why: 'Mutations are random changes to DNA that can be neutral, harmful, or occasionally beneficial, providing raw genetic variation for natural selection to act on.' },
    { level: 55, q: 'The rate at which an object changes its velocity is called...', a: 'Acceleration', d: ['Acceleration', 'Momentum', 'Displacement', 'Force'], why: 'Acceleration measures how quickly velocity changes over time, whether that change is in speed, direction, or both.' }
    ];
