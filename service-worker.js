/* Offline cache + push delivery for Nervexus. Network-first so a reload always gets
   ...
   (see supabase/functions/_shared/push.ts) and shows it even with the app fully
   closed, and relays notification/action-button clicks back into the app (or
   relaunches it with a deep link if nothing's open). Bump CACHE on every shell
   change so activate() purges the old cache and clients pick up fresh assets. */
const CACHE = 'cc-v3';
const SHELL = [
...
// Payload shape comes from supabase/functions/_shared/push.ts's PushPayload —
...
self.addEventListener('push', (e) => {
  let payload = {};
  try { payload = e.data ? e.data.json() : {}; } catch (err) { payload = { title: 'Nervexus', body: e.data ? e.data.text() : '' }; }
  const title = payload.title || 'Nervexus';
  ...
  const options = {
    ...
    actions: (payload.actions || []).slice(0, 3).map((a) => ({ action: a.action, title: a.title })), // most platforms cap at ~3 buttons
  };
  e.waitUntil(
